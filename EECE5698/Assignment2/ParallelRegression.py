...
def estimateGrad(fun,x,delta):
     ...
     Testing on values given in x.
     """
     d = len(x)             #scalar d: size of x
     grad = np.zeros(d)     #creates a row vector size of d filled with zeros
     for i in range(d):     #from 0 to length of vector x
         e = np.zeros(d)    #creates another row vector of size d filled with zeros
         e[i] = 1.0         #set the ith element to 1.0
...
def predict(x,beta):
    ...
    """
    return np.inner(x,np.transpose(beta))
...
def f(x,y,beta):
    ...
    """
    return (y - np.inner(x,np.transpose(beta)))**2
...
def localGradient(x,y,beta):
    ...
    """
    return -2 * (y - np.inner(x,np.transpose(beta))) * x
    #should this x be transpose? After testing, neither may be necessary
...
def F(data,beta,lam = 0):
    ...
    """
    n = data.count()
    mean_sqaure_sum = data.map(lambda (x,y): f(x,y,beta)).reduce(add) / n
    lam_beta_sum = lam * sum(beta**2)
    return mean_sqaure_sum + lam_beta_sum
...
def gradient(data,beta,lam = 0):
    ...
    """
    n = data.count()
    grad_sum = data.map(lambda (x,y): np.array(localGradient(x,y,beta)))\
                   .reduce(add) / n
    lam_beta_sum = 2 * lam * beta
    return grad_sum + lam_beta_sum
...
def train(data,beta_0, lam,max_iter,eps):
    ...
    while(k < max_iter):
        grad = gradient(data,beta,lam)                                #grad of our current iteration
        gain = lineSearch(lambda x: F(data,x,lam),beta,grad)          #find a gain to update beta
        beta = beta - (gain * grad)                                   #update beta
        meansq = F(data,beta,lam)                                     #calculate new MSE
        norm_grad = np.sqrt(sum([elem**2 for elem in grad]))          #l2 norm of gradient
        time_elapsed = time() - start_time                            #time since start
        print "Iteration: ", k, "   Time Elapsed: ", time_elapsed, \
        "   F(Betak): ", meansq, "   Gradient Norm: ", norm_grad
        if norm_grad < eps:
            return (beta, norm_grad, k)                               #returning vals in a tuple
        k+=1
...
if __name__ == "__main__":
	...
	print 'Computing MSE on data',args.testdata
        MSE = test(data,beta)
	print 'MSE is:', MSE
    #append MSE and time_elapsed outputs to a file (bash scripting)
    outputfile = open("MSE.o", 'a')
    print>>outputfile, MSE
    outputfile.close()
    #outputfile = open("TIME.o", 'a')
    #print>>outputfile, time_elapsed
    #outputfile.close()
